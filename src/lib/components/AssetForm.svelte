<script>
  ...
  let { asset: assetProp = defaultAsset, isUpdate = false, form } = $props();
  let asset = $state({ ...assetProp });
...
<div class="mx-auto mb-4 w-full max-w-screen-md rounded-md bg-white/90 p-4 shadow">
  <form
    method="POST"
