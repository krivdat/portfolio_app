<script>
    ...
    import { invalidateAll, redirect } from '$app/navigation';
    ...
    $: asset = {
        ...asset,
        purchase_date: formatDate(asset.purchase_date, 'en-CA', { year: 'numeric', month: '2-digit', day: '2-digit' })  // Format for input type="date"
    };
    ...
    async function handleDelete() {
        ...
        if (confirmed) {
            try {
                ...
                await invalidateAll();
                throw redirect(302, '/assets'); // Redirect after successful deletion
            } catch (error) {
...
<form method="POST" action="?/update" use:enhance>
    ...
    <input type="number" id="purchasePrice" name="purchasePrice" value={asset.purchase_price} required />
